...
###########################################################################################
    ...
    pivot_table['pkg_staged_utc_ts_cum'] = pivot_table['pkg_staged_utc_ts'].cumsum()

    pivot_table['Staged'] = pivot_table['pkg_staged_utc_ts_cum']
    ...
    for day, group in pivot_table.groupby('custom_day'):
        ...
        group['pkg_staged_utc_ts_cum'] = group['pkg_staged_utc_ts'].cumsum() 

        group['Stage'] = group['pkg_staged_utc_ts_cum']

        plt.plot(group['timestamp'], group['Stage'], label = 'Stage', color = 'purple')
        plt.axhline(y=max_stage, color='purple', linestyle='--', label = 'Max Stage')
        ...
        pkg_staged_utc_ts_cum = group['Staged'].iloc[-1]
        print(f'Total Staged: {pkg_staged_utc_ts_cum}')
        daily_dic[day] = pkg_staged_utc_ts_cum
