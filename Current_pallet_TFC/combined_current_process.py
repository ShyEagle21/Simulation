...
###################################################################################
class Sortation_Center_Original_Pallet:
    def __init__(self, 
                env, 
                pallets_df, 
                USPS_Fluid_Status,
...
class Sortation_Center_Original_Fluid:
    def __init__(self, 
                env, 
                pallets_df, 
                USPS_Fluid_Status,
...
def setup_simulation(day_pallets, 
    ...
    current_resources = {'tm_pit_unload': simpy.Resource(env, capacity=night_tm_pit_unload), 
                        ...
                        'tm_TLMD_induct_stage': simpy.PriorityResource(env, capacity=night_tm_TLMD_induct_stage),
    ...
    sortation_center = Sortation_Center_Original_Pallet(env, 
                                        day_pallets, 
                                        USPS_Fluid_Status,
                                        UPSN_Fluid_Status,
                                        FDEG_Fluid_Status,
    ...
    env_f.process(manage_resources(env, 
                                 sortation_center,
                                 current_resources,
                                 night_tm_pit_unload, 
                                 night_tm_pit_induct, 
    ...
    current_resources_f = {'tm_pit_unload': simpy.Resource(env_f, capacity=night_tm_pit_unload), 
                        'tm_pit_induct': simpy.PriorityResource(env_f, capacity=night_tm_pit_induct), 
                        'tm_nonpit_split': simpy.Resource(env_f, capacity=night_tm_nonpit_split), 
                        'tm_nonpit_NC': simpy.PriorityResource(env_f, capacity=night_tm_nonpit_NC), 
                        'tm_nonpit_buffer': simpy.PriorityResource(env_f, capacity=night_tm_nonpit_buffer),
                        'tm_TLMD_induct': simpy.PriorityResource(env_f, capacity=night_tm_TLMD_induct),
                        'tm_TLMD_induct_stage': simpy.PriorityResource(env_f, capacity=night_tm_TLMD_induct_stage),
                        'tm_TLMD_picker': simpy.Resource(env_f, capacity=night_tm_TLMD_picker),
                        'tm_TLMD_sort': simpy.Resource(env_f, capacity=night_tm_TLMD_sort),
                        'tm_TLMD_stage': simpy.Resource(env_f, capacity=night_tm_TLMD_stage)}
    
    sortation_center_f = Sortation_Center_Original_Fluid(env_f, 
                                        day_pallets, 
                                        USPS_Fluid_Status,
                                        UPSN_Fluid_Status,
                                        FDEG_Fluid_Status,
    ...
    # Start tracking metrics and schedule arrivals
    env.process(sortation_center_f.track_metrics())
    sortation_center_f.schedule_arrivals()
    env.process(linehaul_C_arrival(env_f,sortation_center_f))
    env.process(TFC_arrival(env_f,sortation_center_f))
    env.process(between_shift(env_f, sortation_center_f))
    env.process(break_1(env_f, sortation_center_f))
    env.process(break_2(env_f, sortation_center_f))
    env.process(break_3(env_f, sortation_center_f))
    env.process(break_4(env_f, sortation_center_f))

    # for start, end in unavailable_periods:
    #     env.process(make_resources_unavailable(env, sortation_center, start, end))

    env.process(manage_resources(env_f, 
                                 sortation_center_f,
                                 current_resources,
                                 night_tm_pit_unload, 
                                 night_tm_pit_induct, 
    ...
    return env, env_f, sortation_center, sortation_center_f
...
def Simulation_Machine(predict,
    ...
    env, env_f, sortation_center, sortation_center_f = setup_simulation(pallet_info, 
                                             night_tm_pit_unload, 
                                             night_tm_pit_induct, 
                                             night_tm_nonpit_split, 
                                             night_tm_nonpit_NC, 
    ...
    return results, results_f, results_var_05, results_var_10, results_var_15, results_var_20, results_var_25, results_var_30, results_var_35, results_var_40,  df_package_distribution, TFC_arrival_minutes
